...
class SFCourse{
    ...
    public function getCourses(){
		$query = "SELECT Id, Name, Type__c, Start_Date__c, Finish_Date__c, Price__c, Venue__c, Course_External_ID__c, RefCouse_Event__c, Price_OAP_Student__c, Description__c, Fully_Booked__c, Status__c from Course_Event__c WHERE Display_on_website__c='Yes'";
		...
		foreach ($response->records as $record) {
			$mycourse = new Course($record->Name,$record->RefCouse_Event__c,$record->Start_Date__c, $record->Finish_Date__c,
								   ...
								   $record->Price_OAP_Student__c,$record->Description__c, $record->Id, $record->Course_External_ID__c,$record->Type__c, 
										$record->Fully_Booked__c,$record->Status__c);
